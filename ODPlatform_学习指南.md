...
ODPlatform 采用**模块化架构**而非严格分层。CLI 入口直接调用各业务模块，没有中间"服务层"或"核心层"抽象：

```
CLI 层 (cli/)             ← 用户交互入口，直接调用业务模块
  │
  ...
  ├── inference/          ← 模型推理
  ├── webui/              ← Gradio Web 界面
  ├── config/             ← CLI 配置管理（代理到 run_config/）
  └── common/             ← 被所有模块依赖的基础设施
```
...
common/ (基础设施 — 被所有模块依赖)
  ...
  ▼
业务模块（单向依赖链）：
  data_pipeline → data_validation → training → evaluation
       ↓               ↓               ↓           ↓
      CLI             CLI             CLI         CLI
                                            training → inference（训练完自动推理验证）
                                            CLI/WebUI → inference（日常检测推理）

run_config/（配置管理 — 被 CLI 和 training 依赖，独立于业务链）
```

- `common/` 被所有模块依赖，不依赖任何业务模块（纯基础工具）
- 业务模块依赖是单向的：数据管道 → 数据校验 → 训练 → 评估
- `run_config/` 是独立的配置管理模块，不参与业务链
- `cli/` 是入口，负责解析参数、调用各业务模块、组合结果
...
odp-config     = "odp_platform.cli.config_cli:main"
...
odp-backend    = "odp_platform.cli.backend:main"
```

| 命令 | 功能 | 入口函数 |
|------|------|---------|
| `odp-init` | 创建运行时目录结构 | `init_project:initialize_project` |
| `odp-reset` | 清理运行时数据 | `reset_project:main` |
| `odp-transform` | 数据格式转换（VOC/COCO/LabelMe ↔ YOLO） | `transform_data:main` |
| `odp-validate` | 数据质量校验 | `validate_data:main` |
| `odp-config` | 配置生成/快照/溯源 | `config_cli:main` |
| `odp-train` | 模型训练 | `train:main` |
| `odp-val` | 模型评估 | `val:main` |
| `odp-infer` | 模型推理（detect/live 子命令） | `infer:main` |
| `odp-webui` | 启动 Gradio Web 界面 | `webui.app:main` |
| `odp-backend` | 启动 FastAPI 后端 | `cli.backend:main` |
...
```
docs/architecture/
├── ADR-001-monorepo.md               # Monorepo 布局决策
├── ADR-002-paths-strategy.md         # 路径探测策略（marker 模式）
├── ADR-002-data-pipeline.md          # 数据管道设计
├── ADR-003-naming.md                 # 命名规范
├── ADR-004-data-validation.md        # 数据校验架构
├── ADR-013-run-config.md             # 运行配置管理
├── ODPlatform_ADR_V1_0.md            # ADR V1.0 汇总
└── ODPlatform_项目结构设计指南_V1_0.pdf  # 项目结构设计指南
```
...
- **用户模式**（6 个 Tab）：面向日常检测操作人员
  - 单图检测 / 文件夹检测 / 视频检测 / 实时摄像头 / 模型选择 / LLM 对话
- **管理员模式**（11 个 Tab）：在用户模式基础上，点击齿轮图标输入密码后增加
  - 用户全部 6 个 Tab + Dashboard / 模型演示 / 数据集浏览 / 训练 / 数据校验 / 配置管理
...
### 8.3 RSOD / VisDrone Baseline 实验结果

项目内置了两个遥感目标检测 baseline 实验（解压自 `experiments.rar`）：

| 实验 | 数据集 | 模型 | 说明 |
|------|--------|------|------|
| `rsod_baseline` | RSOD (遥感) | YOLO11n | 遥感旋转目标检测 baseline，包含 mAP 曲线、PR 曲线、混淆矩阵 |
| `visdrone_baseline` | VisDrone (无人机) | YOLO11n | 无人机视角目标检测 baseline，含类别分布、训练损失曲线 |

每个实验目录在 `data/runs/experiments/<name>/` 下包含：
- `results.png` / `BoxPR_curve.png` / `BoxF1_curve.png` — 训练 + 评估曲线
- `confusion_matrix.png` / `labels.jpg` — 混淆矩阵 + 类别分布
- `results.csv` — 逐轮指标数据
- `weights/best.pt` / `last.pt` — 最佳 / 最后权重
...
`odp_platform/inference/frame_source/` 是独立的帧输入抽象层，可整包拷贝到其他项目：
...
配置管理由 `odp_platform/run_config/` 模块实现（独立于业务链）：
...
├── schema.py             配置模式定义
├── loader.py             配置加载器（文件/环境变量/CLI 参数合并）
├── merger.py             多源配置合并策略
├── template.py           配置模板生成
├── validator.py          配置校验器
└── fields/               各任务字段定义
...
CLI 入口 `odp-config` 是 `run_config/` 的代理层（在 `cli/config_cli.py`），负责命令行交互。`training/experiment.py` 直接导入 `run_config/` 的 API 生成训练配置。
...
1. **单向依赖**：业务模块只能沿 `data_pipeline → data_validation → training → evaluation` 方向依赖
2. **common/ 是纯基础库**：不依赖任何业务模块，但被所有模块依赖
3. **CLI 是唯一入口**：业务模块不互相直接调用，统一由 CLI 编排
4. **禁止**：反向依赖、循环依赖、`sys.path.append`（安装模式正常时不需要）
